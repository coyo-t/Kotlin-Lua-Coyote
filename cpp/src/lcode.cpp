...
void luaK_ret(FuncState *fs, int first, int nret)
{
	...
	switch (nret)
	{
		case 0: op = OP_RETURN0;
			break;
		case 1: op = OP_RETURN1;
			break;
...
int luaK_code(FuncState *fs, Instruction i)
{
	...
	f->code = luaM::growvector<Instruction>(
		fs->ls->L,
		f->code,
		fs->pc,
		&f->sizecode,
...
static int addk(FuncState *fs, TValue *key, TValue *v)
{
	...
	f->k = luaM::growvector<TValue>(L, f->k, k, &f->sizek, MAXARG_Ax, "constants");
...
static int luaK_exp2K(FuncState *fs, expdesc *e)
{
	if (!hasjumps(e))
	{
		...
		{
			/* move constants to 'k' */
			case VTRUE: info = boolT(fs);
				break;
			case VFALSE: info = boolF(fs);
				break;
			case VNIL: info = nilK(fs);
				break;
			case VKINT: info = luaK_intK(fs, e->u.ival);
				break;
			case VKFLT: info = luaK_numberK(fs, e->u.nval);
				break;
			case VKSTR: info = stringK(fs, e->u.strval);
				break;
			case VK: info = e->u.info;
				break;
