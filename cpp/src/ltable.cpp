...
static Node *mainpositionTV(const Table *t, const TValue *key)
{
	switch (ttypetag(key))
	{
		...
		case LUA_VLNGSTR: {
			TString *ts = tsvalue(key);
			return hashpow2(t, luaS::hashlongstr(ts));
		}
		case LUA_VFALSE:
...
static int equalkey(const TValue *k1, const Node *n2, int deadok)
{
	...
	switch (keytt(n2))
	{
		...
		case ctb(LUA_VLNGSTR):
			return luaS::eqlngstr(tsvalue(k1), keystrval(n2));
		default:
						...
						unsigned int nhsize)
{
	...
	if (l_unlikely(newarray == nullptr && newasize > 0))
	{
		...
		luaD::lthrow(L, LUA_ERRMEM); /* raise error (with array unchanged) */
	}
...
lua_Unsigned luaH_getn(Table *t)
{
	...
	if (limit > 0 && isempty(&t->array[limit - 1]))
	{
		...
		{
			/* must search for a boundary in [0, limit] */
