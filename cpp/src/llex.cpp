...
#define save_and_next(ls) (save(ls, ls->current), ls->next())
...
static l_noret lexerror(LexState *ls, const char *msg, int token)
{
	...
	luaD::lthrow(ls->L, LUA_ERRSYNTAX);
}
						...
						int firstchar)
{
	...
	/* initialize buffer */
	ls->buff->resizebuffer(ls->L, LUA_MINBUFFER);
	...
	for (;;)
	{
		switch (ls->current)
		{
			...
			case '\r': {
				...
				inclinenumber(ls);
				if (!seminfo)
					ls->buff->resetbuffer(); /* avoid wasting space */
				break;
			}
			default: {
				if (seminfo)
...
static int readhexaesc(LexState *ls)
{
	int r = gethexa(ls);
	r = (r << 4) + gethexa(ls);
	ls->buff->buffremove(2); /* remove saved chars from buffer */
	return r;
}
...
static unsigned long readutf8esc(LexState *ls)
{
	...
	esccheck(ls, ls->current == '}', "missing '}'");
	ls->next(); /* skip '}' */
	ls->buff->buffremove(i); /* remove saved chars from buffer */
	return r;
}
...
static int readdecesc(LexState *ls)
{
	...
	ls->buff->buffremove(i); /* remove read digits from buffer */
	return r;
}
		...
		switch (ls->current)
		{
			...
			case '-': {
				...
				if (ls->current == '[')
				{
					...
					ls->buff->resetbuffer(); /* 'skip_sep' may dirty the buffer */
					if (sep >= 2)
					{
						read_long_string(ls, NULL, sep); /* skip long comment */
						ls->buff->resetbuffer(); /* previous call may dirty the buff. */
						break;
					}
				}
				/* else short comment */
				while (!ls->currIsNewline() && ls->current != EOZ)
